...
struct Maze {
    ...
    vector<int> dfs(int node) {
        ...
        vector<int> connectedManholes; // Manhole ids connected to the node
        ...
        if (graph[node].type == 1) connectedManholes.push_back(node);
        ...
        for (int neighbor : graph[node].neighbors) {
            if (!visited[neighbor]) {
                ...
                vector<int> subIds = dfs(neighbor);
                connectedManholes.insert(connectedManholes.end(), subIds.begin(), subIds.end());

                // Update the low time
                low[node] = min(low[node], low[neighbor]);
                if (low[neighbor] > disc[node]) bridges.emplace_back(make_pair(node, neighbor), subIds);
